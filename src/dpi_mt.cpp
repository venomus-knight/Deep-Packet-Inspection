...
struct Stats {
  ...
  std::unordered_map<std::string, std::string> detected_ja3; // hash -> app name

  void recordApp(AppType app, const std::string &sni,
                 const std::string &ja3_hash = "",
                 const std::string &ja3_app = "") {
...
private:
  ...
  void run() {
    while (running_) {
      ...
      // Feed to IDS (thread-safe)
      ids_->processPacket(pkt.tuple.src_ip, pkt.tuple.dst_ip,
                          pkt.tuple.dst_port, pkt.tcp_flags, pkt.ts_sec);
      ...
      // Record stats
      stats_->recordApp(flow.app_type, flow.sni, flow.ja3_hash, flow.ja3_app);
  ...
  void classifyFlow(Packet &pkt, FlowEntry &flow) {
    ...
    if (pkt.tuple.dst_port == 443 && pkt.payload_length > 5) {
      ...
      if (flow.ja3_hash.empty()) {
        auto ja3 = JA3Fingerprint::fingerprint(payload, pkt.payload_length);
        if (ja3) {
...
public:
  ...
  DPIEngine(const Config &cfg) : config_(cfg) {
    ...
    for (int i = 0; i < total_fps; i++) {
      fps_.push_back(std::make_unique<FastPath>(i, &rules_, &stats_,
                                                &output_queue_, &ids_));
    }
...
private:
  ...
  void printReport() {
    ...
    // JA3 Fingerprints
    if (!stats_.detected_ja3.empty()) {
      std::cout << "\n[JA3 TLS Fingerprints]\n";
      for (const auto &[hash, app] : stats_.detected_ja3) {
        std::cout << "  - " << hash;
        if (!app.empty()) {
          std::cout << " -> " << app;
        } else {
    ...
    if (alerts.empty()) {
      ...
      std::cout << "  Total alerts: " << ids_.totalAlerts() << "\n";
    }
  }
};
